...
from app.user_store import register_user, get_user_by_username
from app.auth_logger import log_auth_request
...
def create_app() -> Flask:
    ...
    # Seed two keys immediately
    seed_keys_if_empty()
    
    app = Flask(__name__)
    app.config["JSON_SORT_KEYS"] = False  # keep JWKS order readable
    
    # Initialize rate limiter with stricter settings
    limiter = Limiter(
        app=app,
        key_func=lambda: "global",  # Global rate limit for all requests
        default_limits=[],
        storage_uri="memory://",
        headers_enabled=True,
        swallow_errors=False  # Don't ignore rate limit errors
    )
    ...
    def register():
        """
        POST /register
        Accepts user registration details and generates a secure password.
        Returns the password in JSON format.
        """
        try:
            ...
            # Register user and get generated password
    ...
    def auth():
        ...
        - Logs authentication request to auth_logs table (only on success)
        """
        # Get request IP for logging
        request_ip = get_remote_address()
        user_id = None
        
        # Try to get username from request (if provided)
        try:
            ...
            # If username lookup fails, continue without user_id
            pass
        ...
        # Log successful authentication request (only on success)
